...
//_____________________________________________________________________________
Bool_t
DCAnalyzer::TrackSearchTPCRiemann()
{

  if (!tpcPR->Read(m_TPCHitCont)) {    
...
Bool_t
DCAnalyzer::TrackSearchTPCGenFit(HypTPCTask &fGFTask, Bool_t useRave)
{
  for (auto helix : m_TPCTC_Riemann) {
    if (!helix->IsHelix()) continue;
    TPCTrack *tpcTrack = new TPCTrack();    
    tpcTrack->SetHelixTrack(helix);
    tpcTrack->SetCharge(helix->Charge());
    tpcTrack->SetHelixTrackID(helix->GetTrackID());
    m_TPCTC_GenFit.push_back(tpcTrack);
    
    fGFTask.AddHelixTrack(211,helix);
  }
  
  fGFTask.FitTracks();
  fGFTask.FinalizeGFTracks(m_TPCTC_GenFit);

  if (useRave) {
    HypTPCVertex *vertex = new HypTPCVertex();
    vertex->Execute(m_TPCTC_GenFit, m_TPCVC_Rave);
...
{  
  ...
  del::ClearContainer(m_TPCTC_GenFit);
  del::ClearContainer(m_TPCVC_Rave);
}
