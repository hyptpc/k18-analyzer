...
void HypTPCTask::FinalizeGFTracks(std::vector<TPCTrack*> &tpcTrackArray) {

  Int_t nTrack = GetNTrack();  
  for (int n = 0 ; n < nTrack ; n++) {

    TPCTrack *tpcTrack = tpcTrackArray.at(n);

    auto gfTrack =  GetFittedTrack(n);
    auto status = TrackCheck(n);

    tpcTrack->SetStatus(status);
    tpcTrack->SetGenFitTrack(gfTrack);
    tpcTrack->SetGFTrackID(n);

    if (status == 0) continue;

    auto numCluster = GetNHits(n);
    auto charge = GetCharge(n);
    auto chisqr = GetChi2NDF(n);
    auto pvalue = GetPvalue(n);
    auto tof = GetTrackTOF(n);
    auto tracklength = GetTrackLength(n);

    tpcTrack->SetNumCluster(numCluster);
    tpcTrack->SetGFCharge(charge);
    tpcTrack->SetChisqr(chisqr);
    tpcTrack->SetPvalue(pvalue);
    tpcTrack->SetTrackTOF(tof);
    tpcTrack->SetTrackLength(tracklength);

    TVector3 posTgt, momTgt;
    Double_t TrackLengthTgt, TimeOfFlightTgt;
    Bool_t extrapTgt = ExtrapolateToTarget(n, posTgt, momTgt, TrackLengthTgt, TimeOfFlightTgt);
    if (extrapTgt) {
      tpcTrack->SetPosAtTarget(posTgt);
      tpcTrack->SetMomentumTarget(momTgt);
    }
    
    TVector3 posHTOF, momHTOF;
    Double_t TrackLengthHTOF, TimeOfFlightHTOF;
    Bool_t extrapHTOF = ExtrapolateToHTOF(n, posHTOF, momHTOF, TrackLengthHTOF, TimeOfFlightHTOF);

    if (extrapHTOF){
      tpcTrack->SetPosAtHTOF(posHTOF);
      tpcTrack->SetMomentumHTOF(momHTOF);
      tpcTrack->SetTrackLengthHTOF(TrackLengthHTOF);
      tpcTrack->SetTrackTOFHTOF(TimeOfFlightHTOF);
    }
  }
...
/*
TVector3 HypTPCTask::GetTrackParameter(int trackid) const{
  ...
  try {
    gfTrack = GetFittedTrack(trackid);
...
void HypTPCTask::GetMomentumWithVertex(genfit::Track *gfTrack, TVector3 posVertex, TVector3 &momVertex, TVector3 &pocaVertex) const
{
  ...
  try {
    trackRep -> extrapolateToPoint(fitState, 0.1*posVertex);
  } catch (genfit::Exception &e) {
  }
