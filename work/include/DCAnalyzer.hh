...
typedef std::vector<TPCTrack*> TPCTrackContainer;
...
private:
  ...
  TPCTrackContainer                  m_TPCTC_GenFit;
  TPCVertexContainer                 m_TPCVC_Rave;
...
public:
  ...
  Bool_t TrackSearchTPCRiemann();

  Bool_t TrackSearchTPCGenFit(HypTPCTask &fGFTask, Bool_t useRave = true);
  ...
  Int_t GetNTracksTPCGenFit() const { return m_TPCTC_GenFit.size(); }
  Int_t GetNVerticesTPCRave() const { return m_TPCVC_Rave.size(); }
  ...
  TPCTrack* GetTrackTPCGenFit(Int_t l) const {
    return m_TPCTC_GenFit.at(l);
  }
  TPCVertex* GetVertexTPCRave(Int_t l) const {
    return m_TPCVC_Rave.at(l);
  }

  TPCRiemannTrackContainer GetTrackTPCRiemann() const {return m_TPCTC_Riemann;}
  TPCTrackContainer GetTrackTPCGenFit() const {return m_TPCTC_GenFit;}

  TPCVertexContainer GetVertexTPC() const {return m_TPCVC_Rave;}
