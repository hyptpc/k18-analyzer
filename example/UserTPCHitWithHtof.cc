...
std::vector<Double_t> htofua;
std::vector<Double_t> htofda;
std::vector<Double_t> htofsa;
std::vector<std::vector<Double_t>> htofut;
std::vector<std::vector<Double_t>> htofdt;
std::vector<std::vector<Double_t>> htofst;
}
...
ProcessNormal()
{
  ...
  { // HTOF 
    auto n = NameHodo[kHTOF];
    for(const auto& hit: rawData.GetHodoRawHC(n)){      
      htofseg.push_back(hit->SegmentId());
      htofua.push_back(hit->GetAdcUp());
      htofda.push_back(hit->GetAdcDown());
      htofsa.push_back(hit->GetAdcExtra());
      htofut.push_back(hit->GetArrayTdcUp());
      htofdt.push_back(hit->GetArrayTdcDown());
      htofst.push_back(hit->GetArrayTdcExtra());
      // hit->Print();
    }
  }
...
ConfMan::InitializeHistograms()
{
  ...
  tree->Branch("htof_raw_seg", &htofseg);
  tree->Branch("htof_adc_u", &htofua);
  tree->Branch("htof_adc_d", &htofda);
  tree->Branch("htof_adc_s", &htofsa);
  tree->Branch("htof_tdc_u", &htofut);
  tree->Branch("htof_tdc_d", &htofdt);
  tree->Branch("htof_tdc_s", &htofst);
