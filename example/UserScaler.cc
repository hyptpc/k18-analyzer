...
#if !MAKE_LOG
  if(gUnpacker.get_event_number()%200==0)
    gScaler.Print();
#endif
...
ConfMan::InitializeHistograms()
{
  ...
  // please do not use a white space character.
  {
    ...
    Int_t r = 0;
    gScaler.Set(c, r++, ScalerInfo("BHT",        0,  6));
    gScaler.Set(c, r++, ScalerInfo("T0",         0, 13));
    gScaler.Set(c, r++, ScalerInfo("BH2",        0,  7));
    gScaler.Set(c, r++, ScalerInfo("BAC",        0,  8));
    gScaler.Set(c, r++, ScalerInfo("KVC1",       0, 11));
    gScaler.Set(c, r++, ScalerInfo("T1",         0, 52));
    gScaler.Set(c, r++, ScalerInfo("HTOF",       0,  9));
    gScaler.Set(c, r++, ScalerInfo("HTOF-Mp2",   0, 48));
    gScaler.Set(c, r++, ScalerInfo("HTOF-Mp3",   0, 49));
    gScaler.Set(c, r++, ScalerInfo("HTOF-Fwd",   0, 53));
    gScaler.Set(c, r++, ScalerInfo("BEAM-A",     0, 19));
    gScaler.Set(c, r++, ScalerInfo("BEAM-B",     0, 20));
    gScaler.Set(c, r++, ScalerInfo("BEAM-C",     0, 21));
    gScaler.Set(c, r++, ScalerInfo("BEAM-D",     0, 22));
    gScaler.Set(c, r++, ScalerInfo("BEAM-E",     0, 23));
    gScaler.Set(c, r++, ScalerInfo("BEAM-F",     0, 24));
  }

  {
    ...
    gScaler.Set(c, r++, ScalerInfo("TM",           0,  50));
    gScaler.Set(c, r++, ScalerInfo("SY",          -1, -1));
    gScaler.Set(c, r++, ScalerInfo("K-Beam",       0, 30));
    gScaler.Set(c, r++, ScalerInfo("Pi-Beam",      0, 29));
    gScaler.Set(c, r++, ScalerInfo("Beam",        -1, -1));
    gScaler.Set(c, r++, ScalerInfo("CVC",          0, 12));
    gScaler.Set(c, r++, ScalerInfo("SAC3",         0, 10));
    gScaler.Set(c, r++, ScalerInfo("SFV",          0, 51));
    gScaler.Set(c, r++, ScalerInfo("Clock-PS",      0, 40));
    gScaler.Set(c, r++, ScalerInfo("TRIG-A",        0, 25));
    gScaler.Set(c, r++, ScalerInfo("TRIG-B",        0, 26));
    gScaler.Set(c, r++, ScalerInfo("TRIG-C",        0, 27));
    gScaler.Set(c, r++, ScalerInfo("TRIG-D",        0, 28));
    gScaler.Set(c, r++, ScalerInfo("TRIG-E",        0, 29));
    gScaler.Set(c, r++, ScalerInfo("TRIG-F",        0, 30));
  }

  {
    ...
    // gScaler.Set(c, r++, ScalerInfo("L2-Req",        0,  5));
    gScaler.Set(c, r++, ScalerInfo("L2-Acc",        0,  5));
    gScaler.Set(c, r++, ScalerInfo("Level1-PS",     0, 42));
    gScaler.Set(c, r++, ScalerInfo("TRIG-A-PS",     0, 32));
    gScaler.Set(c, r++, ScalerInfo("TRIG-B-PS",     0, 33));
    gScaler.Set(c, r++, ScalerInfo("TRIG-C-PS",     0, 34));
    gScaler.Set(c, r++, ScalerInfo("TRIG-D-PS",     0, 35));
    gScaler.Set(c, r++, ScalerInfo("TRIG-E-PS",     0, 36));
    gScaler.Set(c, r++, ScalerInfo("TRIG-F-PS",     0, 37));
    gScaler.Set(c, r++, ScalerInfo("TRIG-PSOR-A",   0, 38));
    gScaler.Set(c, r++, ScalerInfo("TRIG-PSOR-B",   0, 39));
    gScaler.Set(c, r++, ScalerInfo("Reserve2-PS",   0, 41));
  }
...
ConfMan::FinalizeProcess()
{
  ...
  Double_t l1reqkbeam = gScaler.Fraction("L1-Req", "K-Beam");
  ...
  ofs << std::fixed << std::setprecision(6)
      ...
      << std::left  << std::setw(18) << "L1-Req/K-Beam"   << "\t"
      << std::right << std::setw(12) << l1reqkbeam       << std::endl
      << std::setprecision(0)
