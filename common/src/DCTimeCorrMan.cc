...
namespace{
  ...
  TString chmname[nchamber]={"BLC1a","BLC1b","BLC2a","BLC2b","BPC","BPC"};
...
bool DCTimeCorrMan::Initialize()
{
  ...
  for(int ic=0;ic<nchamber;ic++){
    for(int lay=0;lay<8;lay++){      
      gr=(TGraph*)f->Get(Form("DCTimeCorr_%s_%d",chmname[ic].Data(),lay+1));
      if(!gr) continue;
      //      std::cout<<Form("DCTimeCorr_%s_%d",chmname[ic].Data(),lay+1)<<std::endl;
      key = KEY(chmid[ic],lay,0);
