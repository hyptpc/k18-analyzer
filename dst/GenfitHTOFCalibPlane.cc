...
main( int argc, char **argv )
{
  ...
  GFtracks.DebugMode();
#endif
	TPCLocalTrackHelix* h_temp;
	h_temp->SetCircularCrossTracking(1);
	delete h_temp;
  Int_t ievent = skip;
...
dst::DstRead( int ievent )
{
  ...
  if( ievent%1==0 ){
    ...
    oss << "start : "<<std::put_time(localTime, "%Y-%m-%d %H:%M:%S");
  ...
  event.nsigma_tritonHtof.resize( GFntTpc );
  event.nsigma_deutronHtof.resize( GFntTpc );
  event.nsigma_protonHtof.resize( GFntTpc );
  event.nsigma_kaonHtof.resize( GFntTpc );
  event.nsigma_pionHtof.resize( GFntTpc );
  event.nsigma_electronHtof.resize( GFntTpc );
