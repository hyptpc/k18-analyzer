...
struct Event
{
  ...
  std::vector<Int_t> GFnhHtof;
  std::vector<Double_t> GFxHtof;
  std::vector<Double_t> GFyHtof;
  std::vector<Double_t> GFzHtof;
  std::vector<Double_t> GFtracklenHtof;
...
dst::DstRead( int ievent )
{
  ...
  for( Int_t igf=0; igf<GFntTpc; ++igf ){
    ...
    HF1( genfitHid+7, event.GFmom[igf][0]);
    for( Int_t ihit=0; ihit<nh; ++ihit ){
      ...
      if(event.hitlayer[igf][ihit]>31 ){
	std::cout<<"igf : "<<igf<<" ihit : "<<ihit<<std::endl;
	std::cout<<"hit layer1 : "<<event.hitlayer[igf][ihit]<<std::endl;
	std::cout<<"hit layer2 : "<<layer<<std::endl;
      }
      ...
      event.GFresidual_px[igf][ihit] = mom.x() - event.mom0_x[igf];
      event.GFresidual_py[igf][ihit] = mom.y() - event.mom0_y[igf];
      event.GFresidual_pz[igf][ihit] = mom.z() - event.mom0_z[igf];
    ...
    if(GFtracks.ExtrapolateToHTOF(igf, htofhit, htofmom, htoflen, htoftof)){
      ...
      HF1( genfitHid+24, htoflen);
      HF1( genfitHid+25, htoftof);
      HF1( genfitHid+26, htofhit.x());
      HF1( genfitHid+27, htofhit.y());
      HF1( genfitHid+28, htofhit.z());
      event.GFnhHtof[igf]++;
    }
...
ConfMan::InitializeHistograms( void )
{
  ...
  HB1(genfitHid+24, "[GenFit] Track Length (extrapolated to the HTOF); Length [mm]; Counts [/1 mm]", 500, 0, 500 );
  HB1(genfitHid+25, "[GenFit] Tof (extrapolated to the HTOF); Tof [ns]; Counts [/0.01 ns]", 500, 0, 5 );
  HB1(genfitHid+26, "[GenFit] X (extrapolated to the HTOF); X [mm]; Counts [/0.1 mm]", 500, -25, 25 );
  HB1(genfitHid+27, "[GenFit] Y (extrapolated to the HTOF); Y [mm]; Counts [/0.1 mm]", 500, -25, 25 );
  HB1(genfitHid+28, "[GenFit] Z (extrapolated to the HTOF); Z [mm]; Counts [/0.1 mm]", 500, -143 -25, -143 + 25 );
