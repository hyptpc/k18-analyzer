...
ProcessNormal()
{
  ...
  // Trigger flag
  {
	...
	root::HF1(Form("%s_TDC_%d", name.Data(), seg), tu);
      }
      if(ntu>0){
	root::HF1(Form("%s_Pat", name.Data()), seg);
      }
    }
  }
  ...
  // AC
  {
    ...
    for(Int_t i=0, n=cont.size(); i<n; ++i){
      ...
      for(int it=0, nt=raw->GetSizeTdcUp(); it<nt; ++it){
	Double_t tu = raw->GetTdcUp(it);
        if(gUser.IsInRange("AC_TDC", tu))
          in_range = true;
	root::HF1(Form("%s_TDC_%d", name.Data(), seg), tu);
      }
      for(int it=0, na=raw->GetSizeAdcUp();it<na;it++){
	Double_t au = raw->GetAdcUp(it);
	root::HF1(Form("%s_ADC_%d", name.Data(), seg), au);
        if(in_range)
          root::HF1(Form("%s_AWT_%d", name.Data(), seg), au);
      }
    }
  }
  return true;
...
ConfMan::InitializeHistograms()
{
  { // TriggerFlag
    ...
    for(Int_t i=0; i<NumOfSegTrigFlag; ++i){
      root::HB1(Form("%s_TDC_%d; Channel; Counts", name.Data(), i), mtdcbins);
    }
    root::HB1(Form("%s_Pat; Segment; Counts", name.Data()), patbins);
  }

  { // AC
    TString name = "AC";
    for(Int_t i=0; i<NumOfSegAC; ++i){
      root::HB1(Form("%s_ADC_%d; Channel; Counts", name.Data(), i), adcbins);
      root::HB1(Form("%s_AWT_%d; Channel; Counts", name.Data(), i), adcbins);
      root::HB1(Form("%s_TDC_%d; Channel; Counts", name.Data(), i), tdcbins);
    }
  }
