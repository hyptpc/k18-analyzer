...
class DCAnalyzer : public TPCAnalyzer
{
...
public:
  ...
  //Bool_t DecodeFiberHits(FiberCluster* FiberCl, Int_t layer); //legacy
  //Bool_t DecodeFiberHits(RawData* rawData); //legacy
  ...
  Bool_t TrackSearchSdcOut(const Hodo2HitContainer& HitCont, Bool_t Exclusive=false);
  Bool_t TrackSearchSdcOut(const HodoClusterContainer& ClCont, Bool_t Exclusive=false);
  ...
  void ChiSqrCutKurama(Double_t chisqr);
  ...
  //Combined trackings
  Bool_t TrackSearchBcOutSdcIn(Bool_t Exclusive=false);
  //Bool_t TrackSearchSdcInSdcOut(Bool_t Exclusive=false);
