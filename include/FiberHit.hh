...
public:
  ...
  double GetLeading(Int_t n=0)  const { return m_ud==0? m_raw->GetTdc1(n)  : m_raw->GetTdc2(n);}
  double GetTrailing(Int_t n=0) const { return m_ud==0? m_raw->GetTdcT1(n) : m_raw->GetTdcT2(n);}

  // Call member in this class
  Int_t    GetNPair()       const { return m_pair_cont.size();        }
  double GetTime(Int_t n=0)     const { return m_pair_cont.at(n).time_l;  } // Leading
  double GetCTime(Int_t n=0)    const { return m_pair_cont.at(n).ctime_l; } // Leading
  double GetTimeT(Int_t n=0)    const { return m_pair_cont.at(n).time_t;  } // Trailing
  double GetWidth(Int_t n=0)    const { return m_pair_cont.at(n).tot;     }
  double GetTot(Int_t n=0)      const { return m_pair_cont.at(n).tot;     }

  double GetPosition()  const { return m_position + m_offset;       }

  Int_t    PairId()       const { return m_pair_id;                   }
  //  virtual double SegmentId()    const { return m_segment;                   }

  double GetAdcHG() const { return m_adc_hg; }
  double GetAdcLG() const { return m_adc_lg; }
  double GetMipHG() const { return m_mip_hg; }
  double GetMipLG() const { return m_mip_lg; }
  double GetDeHG() const { return m_dE_hg; }
  double GetDeLG() const { return m_dE_lg; }
