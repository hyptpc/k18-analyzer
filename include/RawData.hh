...
inline const HodoRHitContainer&
//RawData::GetAFTRawHC( int plane, int UorD ) const
RawData::GetAFTRawHC( int plane ) const
{
  if( plane<0 || plane>NumOfPlaneAFT-1 ) plane=0;
  //  if( !(0 <= UorD && UorD <= 1)) plane = 0;
  //  return m_AFTRawHC[2*plane + UorD];
  return m_AFTRawHC[plane];
}

//______________________________________________________________________________

inline const HodoRHitContainer&
RawData::GetSSTRawHC( int plane ) const
{
