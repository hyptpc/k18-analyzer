...
def draw_one_data(c1, name, key, nplane):
  for i in range(nplane):
    c1.cd(i+1) #.SetLogy()
    for j, b in enumerate(mh.beamflag):
      h1 = mh.get(f'{name}_{key}_plane{i}{b}')
      if h1:
...
#______________________________________________________________________________
@mh.update_canvas(divisions=(4, 2))
def draw_one_data2d(c1, name, key, nplane):
  for i in range(nplane):
    c1.cd(i+1).SetLogz()
    hname = (f'{name}_{key}_vs_HitPat_'+
             f'plane{i}{mh.beamflag_for_param}')
    h1 = mh.get(hname)
    if h1:
      h1.Draw('colz')

#______________________________________________________________________________
def draw(name, nplane=0):
  logger.info(f'name={name}, nplane={nplane}')
  ...
  for key in ['TDC', # 'Trailing',
    ...
    for c in ['', 'C']:
      ...
      if c == 'C' and (key == 'TDC' or key == 'TOT'):
        draw_one_data2d(name, key=c+key, nplane=nplane)
