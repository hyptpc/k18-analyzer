...
namespace
{
  ...
  enum EHodoDataType { kHodoLeading, kHodoTrailing, kHodoAdcHi, kHodoAdcLow, kHodoOverflow, kHodoNDataType };
		 ...
		 int id, int plane, int seg, int UorD, int type, int data )
  {
    ...
    //   break;
    // case kHodoAdcHi:
    //   if( UorD==0 ) p->SetAdcHiUp(data);
    //   else          p->SetAdcHiDown(data);
    //   break;
    // case kHodoAdcLow:
    //   if( UorD==0 ) p->SetAdcLowUp(data);
    //   else          p->SetAdcLowDown(data);
    //   break;
    case kHodoLeading:
      if( UorD==0 ) p->SetTdc1(data);
      else          p->SetTdc2(data);
      break;
    case kHodoTrailing:
      if( UorD==0 ) p->SetTdcT1(data);
      else          p->SetTdcT2(data);
      break;
    case kHodoAdcHi:
      if( UorD==0 ) p->SetAdcHi1(data);
      else          p->SetAdcHi2(data);
      break;
    case kHodoAdcLow:
      if( UorD==0 ) p->SetAdcLow1(data);
      else          p->SetAdcLow2(data);
      break;
...
RawData::ClearAll( void )
{
  ...
  del::ClearContainerAll( m_AFTRawHC );
  del::ClearContainerAll( m_SSTRawHC );
...
*/
  ...
  for( int plane=0; plane<NumOfPlaneAFT; ++plane ){    
    //AFT TDC 
    for(int seg = 0; seg<NumOfSegAFT[plane]; ++seg){
      for(int UorD=0; UorD<2; ++UorD){
	//      int nhit_tdc      = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, 0 );
	//      int nhit_trailing = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, 1 );
	int nhit_tdc      = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, kHodoLeading );
	int nhit_trailing = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, kHodoTrailing );
      if( nhit_tdc>0 ){
	for(int i = 0; i<nhit_tdc; ++i){
	  //	  int leading  = gUnpacker.get( DetIdAFT, plane, seg, UorD, 0, i );
	  int leading  = gUnpacker.get( DetIdAFT, plane, seg, UorD, kHodoLeading, i );
	  //	  AddHodoRawHit( m_AFTRawHC[2*plane+UorD], DetIdAFT, plane, seg , UorD, kHodoLeading, leading );
	  AddHodoRawHit( m_AFTRawHC[plane], DetIdAFT, plane, seg , UorD, kHodoLeading, leading );
	}
      }	
      if(nhit_trailing>0){
	for(int i = 0; i<nhit_trailing; ++i){
	  //	  int trailing = gUnpacker.get( DetIdAFT, plane, seg, UorD, 1, i )  ;
	  int trailing = gUnpacker.get( DetIdAFT, plane, seg, UorD, kHodoTrailing, i )  ;
	  //	  AddHodoRawHit( m_AFTRawHC[2*plane+UorD], DetIdAFT, plane, seg , UorD, kHodoTrailing, trailing );
	  AddHodoRawHit( m_AFTRawHC[plane], DetIdAFT, plane, seg , UorD, kHodoTrailing, trailing );
	}	  	
      }
      //if( nhit_tdc==0 )continue; // w/ or w/o TDC,   comment out => pedestal              

      //AFT ADC HIGH
      //      int nhit_adc_hi = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, 2 );
      int nhit_adc_hi = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, kHodoAdcHi );
      if( nhit_adc_hi>0 ){
	for(int i = 0; i<nhit_adc_hi; ++i){
	  //	  int adc_hi  = gUnpacker.get( DetIdAFT, plane, seg, UorD, 2, i )  ;
	  int adc_hi  = gUnpacker.get( DetIdAFT, plane, seg, UorD, kHodoAdcHi, i )  ;
	  //	  AddHodoRawHit( m_AFTRawHC[2*plane+UorD], DetIdAFT, plane, seg , 0, kHodoAdc, adc_hi );//ADC Hi	  
	  AddHodoRawHit( m_AFTRawHC[plane], DetIdAFT, plane, seg , UorD, kHodoAdcHi, adc_hi );//ADC Hi	  
	}
      }
      else continue;
      //AFT ADC LOW
      //      int nhit_adc_low = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, 3 );
      int nhit_adc_low = gUnpacker.get_entries( DetIdAFT, plane, seg, UorD, kHodoAdcLow );
      if( nhit_adc_low>0 ){
	for(int i = 0; i<nhit_adc_low; ++i){
	  //	  int adc_low = gUnpacker.get( DetIdAFT, plane, seg, UorD, 3, i );
	  int adc_low = gUnpacker.get( DetIdAFT, plane, seg, UorD, kHodoAdcLow, i );
	  //	  AddHodoRawHit( m_AFTRawHC[2*plane+UorD], DetIdAFT, plane, seg , 1, kHodoAdc, adc_low );//ADC Low
	  AddHodoRawHit( m_AFTRawHC[plane], DetIdAFT, plane, seg , UorD, kHodoAdcLow, adc_low );//ADC Low
	}
      }
      else continue;      
      }
  ...
  //SST
  for( int plane=0; plane<NumOfPlaneSST; ++plane ){    
    //CFT TDC 
    for(int seg = 0; seg<NumOfSegSST[plane]; ++seg){
      //      int nhit_tdc      = gUnpacker.get_entries( DetIdSST, plane, seg, 0, 0 );
      //      int nhit_trailing = gUnpacker.get_entries( DetIdSST, plane, seg, 0, 1 );
      int nhit_tdc      = gUnpacker.get_entries( DetIdSST, plane, seg, 0, kHodoLeading );
      int nhit_trailing = gUnpacker.get_entries( DetIdSST, plane, seg, 0, kHodoTrailing );
      if( nhit_tdc>0 ){
	for(int i = 0; i<nhit_tdc; ++i){
	  //	  int leading  = gUnpacker.get( DetIdSST, plane, seg, 0, 0, i );
	  int leading  = gUnpacker.get( DetIdSST, plane, seg, 0, kHodoLeading, i );
	  AddHodoRawHit( m_SSTRawHC[plane], DetIdSST, plane, seg, 0, kHodoLeading, leading );
	}
      }	
      if(nhit_trailing>0){
	for(int i = 0; i<nhit_trailing; ++i){
	  //	  int trailing = gUnpacker.get( DetIdSST, plane, seg, 0, 1, i )  ;
	  int trailing = gUnpacker.get( DetIdSST, plane, seg, 0, kHodoTrailing, i )  ;
	  AddHodoRawHit( m_SSTRawHC[plane], DetIdSST, plane, seg, 0, kHodoTrailing, trailing );
	}	  	
      }
      //if( nhit_tdc==0 )continue; // w/ or w/o TDC,   comment out => pedestal              

      //SST ADC HIGH
      //      int nhit_adc_hi = gUnpacker.get_entries( DetIdSST, plane, seg, 0, 2 );
      int nhit_adc_hi = gUnpacker.get_entries( DetIdSST, plane, seg, 0, kHodoAdcHi );
      if( nhit_adc_hi>0 ){
	for(int i = 0; i<nhit_adc_hi; ++i){
	  //	  int adc_hi  = gUnpacker.get( DetIdSST, plane, seg, 0, 2, i )  ;
	  int adc_hi  = gUnpacker.get( DetIdSST, plane, seg, 0, kHodoAdcHi, i )  ;
	  //	  AddHodoRawHit( m_SSTRawHC[plane], DetIdSST, plane, seg , 0, 0, adc_hi );//ADC Hi	  
	  AddHodoRawHit( m_SSTRawHC[plane], DetIdSST, plane, seg, 0, kHodoAdcHi, adc_hi );//ADC Hi	  
	}
      }
      else continue;
      //SST ADC LOW
      //      int nhit_adc_low = gUnpacker.get_entries( DetIdSST, plane, seg, 0, 3 );
      int nhit_adc_low = gUnpacker.get_entries( DetIdSST, plane, seg, 0, kHodoAdcLow );
      if( nhit_adc_low>0 ){
	for(int i = 0; i<nhit_adc_low; ++i){
	  //	  int adc_low = gUnpacker.get( DetIdSST, plane, seg, 0, 3, i );
	  int adc_low = gUnpacker.get( DetIdSST, plane, seg, 0, kHodoAdcLow, i );
	  //	  AddHodoRawHit( m_SSTRawHC[plane], DetIdSST, plane, seg , 1, 0, adc_low );//ADC Low
	  AddHodoRawHit( m_SSTRawHC[plane], DetIdSST, plane, seg, 0, kHodoAdcLow, adc_low );//ADC Low
	}
      }
      else continue;      
    }
