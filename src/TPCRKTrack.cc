...
/*
  For each TPC cluster, a virtual plane normal to the track is defined.
  The X and Y positions of each cluster are treated as two independent points.

  Example: For each HypTPC cluster, the following equations are used
  for the chi-square and NDF calculations:
...
#if 0 //Checking L/R configuration again with RK tracking result.(result was bad)
